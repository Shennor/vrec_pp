...
# принимает имя файла и частоту дискретизации, возвращает mfcc_fbank
...
class Audio:
   ...
   # def __init__(self, cache_dir: str, audio_dir: str = None, sample_rate: int = SAMPLE_RATE, ext='flac'):
    def __init__(self, cache_dir: str, audio_dir: str = None, sample_rate: int = SAMPLE_RATE, ext='wav'):
        ...
        #перебор по всем найденным файлам
        for cache_file in find_files(self.cache_dir, ext='npy'):
            # /path/to/speaker_utterance.npy
            speaker_id = Path(cache_file).stem.split('_')[0]
            utterance_id = Path(cache_file).stem
            self.speakers_to_utterances[speaker_id][utterance_id] = cache_file

    @property
    # озвращает сортированный список говорящих
