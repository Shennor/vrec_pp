...
class LazyTripletBatcher:
    ...
    def get_random_batch(self, batch_size, is_test=False):
        ...
        #assert np.all(np.array([extract_speaker(s) for s in pos_neg[0, :]]) != np.array([extract_speaker(s) for s in pos_neg[1, :]]))
        if np.all(np.array([extract_speaker(s) for s in pos_neg[0, :]]) == np.array([extract_speaker(s) for s in pos_neg[1, :]])):
                print([extract_speaker(s) for s in pos_neg[0, :]])
                print([extract_speaker(s) for s in pos_neg[1, :]])
                print([positive_utterances, negative_utterances])
                assert np.all(np.array([extract_speaker(s) for s in pos_neg[0, :]]) != np.array([extract_speaker(s) for s in pos_neg[1, :]]))
